...
import { 
  ...
  ShieldCheck, FileCheck, Landmark, CheckCircle2, Scale, Trees, Sparkles, Eye, Waves, Utensils
} from 'lucide-react';
import SEO from './SEO';
import buildingImage from '../src/assets/images/regenerated_image_1786989195524.png';
import { useLanguage } from '../context/LanguageContext';
import { editalCommon, editaisData } from '../translations/editais';

interface SantaTeresaJulioOtoni254AuctionPageProps {
  onBack?: () => void;
}

const SantaTeresaJulioOtoni254AuctionPage: React.FC<SantaTeresaJulioOtoni254AuctionPageProps> = () => {
  ...
  const editalEntry = editaisData['santa-teresa-julio-otoni-254'] || editaisData['copacabana-atlantica'];
  const item = editalEntry[language] || editalEntry.pt;

  const canonicalUrl = "https://soaresmartinsadv.com/assessoria-leiloes-judiciais-imoveis-rio-de-janeiro/santa-teresa/casa/rua-julio-otoni-254/";
    ...
    <div className="min-h-screen bg-midnight text-white text-left font-sans">
      ...
      <section className="max-w-7xl mx-auto px-6 pb-12 sm:pb-16">
        <div className="bg-midnight-light/40 backdrop-blur-sm rounded-3xl p-6 sm:p-10 border border-white/10 shadow-2xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-bronze/10 border border-bronze/30 text-bronze text-xs font-semibold uppercase tracking-wider mb-6">
            <Gavel size={14} /> Casa Residencial com 12 Quartos • 203 m² • Santa Teresa/RJ
          </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-6">
                  ...
                  <div className="bg-midnight-light/50 p-4 rounded-xl border border-emerald-500/30">
                    ...
                    <div className="text-sm font-semibold text-emerald-300 mb-2">Lance mínimo de 50%</div>
                    <div className="text-xs text-text-muted mb-1">{tC.minBidLabel}</div>
                ...
                <div className="mt-6 pt-4 border-t border-white/10 space-y-3">
                  ...
                  <a 
                    href="https://wa.me/5521979549241?text=Ol%C3%A1,%20gostaria%20de%20informa%C3%A7%C3%B5es%20sobre%20o%20leil%C3%A3o%20da%20casa%20na%20Rua%20J%C3%BAlio%20Otoni%20254%20em%20Santa%20Teresa."
                    target="_blank"
      ...
      <section className="max-w-7xl mx-auto px-6 pb-16 space-y-12">
        ...
        <article className="bg-midnight-light/30 rounded-3xl p-8 sm:p-12 border border-white/10 text-left space-y-6">
          ...
          {/* Breakdown dos Ambientes e Estrutura */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4">
            {/* Componente 1: Entrada, Recepção e Acesso */}
            <div className="bg-midnight/60 p-6 rounded-2xl border border-white/10 space-y-3">
              <h3 className="text-bronze font-serif font-bold text-base flex items-center gap-2">
                <Trees size={18} /> 1) Acesso Principal e Recepção
              </h3>
              <ul className="text-xs sm:text-sm text-text-muted space-y-2">
                <li><strong className="text-white">Segurança & Privacidade:</strong> Imóvel guarnecido por muro alto, conferindo máxima discrição e privacidade.</li>
                <li><strong className="text-white">Jardim de Entrada:</strong> Acesso inicial por agradável jardim ajardinado que conduz até a recepção e o salão social.</li>
                <li><strong className="text-white">Circulação Interna:</strong> Escadaria central de acesso elegante distribuindo a circulação para os quartos nos pisos superiores e áreas comuns.</li>
              </ul>
            </div>

            {/* Componente 2: Acomodações e Suíte Presidencial */}
            <div className="bg-midnight/60 p-6 rounded-2xl border border-white/10 space-y-3">
              <h3 className="text-bronze font-serif font-bold text-base flex items-center gap-2">
                <Building2 size={18} /> 2) 12 Quartos e Suíte Presidencial
              </h3>
              <ul className="text-xs sm:text-sm text-text-muted space-y-2">
                <li><strong className="text-white">Capacidade Ampla:</strong> 12 quartos de metragens e tipologias diversas com acabamentos de excelente padrão.</li>
                <li><strong className="text-white">Suíte Presidencial:</strong> Unidade máster exclusiva com dimensionamento nobre e vista panorâmica privilegiada.</li>
                <li><strong className="text-white">Estado de Conservação:</strong> Casa em ótimo estado com diversas benfeitorias incorporadas.</li>
              </ul>
            </div>

            {/* Componente 3: Lazer, Piscina e Sauna a Vapor */}
            <div className="bg-midnight/60 p-6 rounded-2xl border border-white/10 space-y-3">
              <h3 className="text-bronze font-serif font-bold text-base flex items-center gap-2">
                <Waves size={18} /> 3) Área de Lazer, Piscina e Sauna
              </h3>
              <ul className="text-xs sm:text-sm text-text-muted space-y-2">
                <li><strong className="text-white">Piscina Panorâmica:</strong> Deck com piscina proporcionando ambiente de relaxamento com vista cinematográfica para o Rio.</li>
                <li><strong className="text-white">Sauna a Vapor:</strong> Instalação completa de sauna a vapor para bem-estar e conforto dos ocupantes e hóspedes.</li>
                <li><strong className="text-white">Solarium & Pátio:</strong> Espaço externo ensolarado integrado à natureza e à mata do entorno.</li>
              </ul>
            </div>

            {/* Componente 4: Bar, Restaurante e Cozinha */}
            <div className="bg-midnight/60 p-6 rounded-2xl border border-white/10 space-y-3">
              <h3 className="text-bronze font-serif font-bold text-base flex items-center gap-2">
                <Utensils size={18} /> 4) Bar, Restaurante e Cozinha
              </h3>
              <ul className="text-xs sm:text-sm text-text-muted space-y-2">
                <li><strong className="text-white">Espaço Bar & Lounge:</strong> Área estruturada para bar com atendimento aos ambientes sociais e de piscina.</li>
                <li><strong className="text-white">Restaurante Integrado:</strong> Salão de refeições planejado para convivência ou operação comercial/hoteleira.</li>
                <li><strong className="text-white">Cozinha Completa:</strong> Cozinha operacional espaçosa com infraestrutura hidráulica e elétrica.</li>
              </ul>
            </div>

            {/* Componente 5: Vista Panorâmica Icônica */}
            <div className="bg-midnight/60 p-6 rounded-2xl border border-white/10 space-y-3">
              <h3 className="text-bronze font-serif font-bold text-base flex items-center gap-2">
                <Eye size={18} /> 5) Vista Deslumbrante da Zona Sul e Cartões-Postais
              </h3>
              <p className="text-xs sm:text-sm text-text-muted leading-relaxed">
                Vista panorâmica indevassável e ampla para a Zona Sul da capital fluminense. É possível avistar de forma espetacular o <strong className="text-white">Cristo Redentor</strong>, o <strong className="text-white">Pão de Açúcar</strong> e a <strong className="text-white">Baía de Guanabara</strong>, tanto das janelas dos quartos quanto da área de lazer e da piscina.
              </p>
            </div>

            {/* Componente 6: Identificação Registral e Cadastral */}
            <div className="bg-midnight/60 p-6 rounded-2xl border border-white/10 space-y-3">
              <h3 className="text-bronze font-serif font-bold text-base flex items-center gap-2">
                <FileCheck size={18} /> 6) Registro Cartorário e Inscrição Municipal
              </h3>
              <ul className="text-xs sm:text-sm text-text-muted space-y-2">
                <li><strong className="text-white">Cartório de Registro:</strong> 9º Ofício do Registro Geral de Imóveis do Rio de Janeiro (9º RGI/RJ).</li>
                <li><strong className="text-white">Matrícula Imobiliária:</strong> Matrícula nº 52318.</li>
                <li><strong className="text-white">Inscrição Municipal (IPTU):</strong> Inscrição nº 0.417.443-9 (Secretaria Municipal de Fazenda/RJ).</li>
                <li><strong className="text-white">Área Construída Edificada:</strong> 203 m².</li>
              </ul>
            </div>
          </div>
        </article>

        {/* Caracterização da Região e Infraestrutura */}
        <article className="bg-midnight-light/30 rounded-3xl p-8 sm:p-12 border border-white/10 text-left space-y-6">
          <div className="flex items-center gap-3 text-bronze">
            <MapPin size={24} className="shrink-0" />
            <h2 className="text-2xl sm:text-3xl font-serif text-white font-bold">
              Caracterização da Região e Infraestrutura Urbana em Santa Teresa
            </h2>
          </div>

          <p className="text-text-muted text-sm sm:text-base leading-relaxed">
            O imóvel na Rua Júlio Otoni, nº 254, situa-se em posição estratégica e elevada no bairro nobre de Santa Teresa, no caminho de acesso para o Parque Nacional da Tijuca e o monumento do Cristo Redentor, com rápida ligação ao bairro vizinho de Laranjeiras.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            <div className="bg-midnight/60 p-6 rounded-2xl border border-white/10 space-y-3">
              <h3 className="text-bronze font-bold text-sm flex items-center gap-2">
                <Sparkles size={16} /> Melhoramentos Públicos Municipais
              </h3>
              <p className="text-xs text-text-muted leading-relaxed">
                A região encontra-se amplamente servida por melhoramentos públicos consolidado pelo município do Rio de Janeiro: rede de distribuição de energia elétrica, telefonia, iluminação pública, pavimentação asfáltica, rede de abastecimento de água tratada e rede de esgotamento sanitário.
              </p>
            </div>

            <div className="bg-midnight/60 p-6 rounded-2xl border border-white/10 space-y-3">
              <h3 className="text-bronze font-bold text-sm flex items-center gap-2">
                <Landmark size={16} /> Mobilidade & Transporte Público
              </h3>
              <p className="text-xs text-text-muted leading-relaxed">
                No tocante a transporte público, há linha de ônibus regular circulando no logradouro com fácil acesso a Laranjeiras e Cosme Velho, além do clássico e tombado Bonde de Santa Teresa, que opera seus trilhos na próxima Rua Almirante Alexandrino.
              </p>
            </div>
          </div>

          <div className="p-6 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl space-y-2">
            <div className="text-emerald-400 font-serif font-bold text-base">
              Oportunidade na 2ª Praça: Aquisição por R$ 1.577.000,00 (50% do Valor de Avaliação)
            </div>
            <p className="text-xs sm:text-sm text-text-muted leading-relaxed">
              Comprar uma propriedade de 203 m² em ótimo estado, com 12 quartos (Suíte Presidencial), piscina, sauna, restaurante/bar e vista cinematográfica para o Cristo Redentor e Pão de Açúcar por R$ 1.577.000,00 na 2ª Praça constitui uma oportunidade singular para moradia de alto padrão ou operação de pousada boutique.
            </p>
          </div>
        </article>
        ...
        <article className="bg-midnight-light/30 rounded-3xl p-8 sm:p-12 border border-white/10 text-left space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-2">
            ...
            <div className="bg-midnight/60 p-6 rounded-2xl border border-white/10 space-y-3">
              ...
              <p className="text-xs text-text-muted leading-relaxed">
                Lances na 1ª praça (21/09/2026 às 12:50h) ou 2ª praça (23/09/2026 às 12:50h). Apresentação de proposta de parcelamento com base no Art. 895 do CPC (25% de sinal + 30 parcelas mensais).
              </p>
            </div>
            ...
            <div className="bg-midnight/60 p-6 rounded-2xl border border-white/10 space-y-3">
              ...
              <p className="text-xs text-text-muted leading-relaxed">
                Assinatura do auto de arrematação, pagamento das guias judiciais e ITBI, expedição da Carta de Arrematação para averbação no 9º RGI/RJ e imissão definitiva na posse.
              </p>
            </div>
          </div>
        </article>
        ...
        <article className="bg-midnight-light/30 rounded-3xl p-8 sm:p-12 border border-white/10 text-left space-y-8">
          ...
          <p className="text-text-muted text-sm sm:text-base leading-relaxed">
            A aquisição de imóvel em leilão judicial demanda rigoroso exame prévio de certidões e dos autos processuais. Nossa assessoria jurídica atua desde a auditoria da Matrícula nº 52318 do 9º RGI/RJ até a imissão formal na posse.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-2">
            ...
            <div className="bg-midnight/60 p-5 rounded-2xl border border-white/10 space-y-2">
              ...
              <p className="text-xs text-text-muted leading-relaxed">
                Registro definitivo no 9º RGI/RJ e imissão do arrematante na posse de todas as dependências do imóvel.
              </p>
            </div>
          </div>
        </article>
        ...
        <section className="bg-bronze p-8 sm:p-12 md:p-14 rounded-3xl text-midnight text-center shadow-2xl border border-bronze/30">
          <div className="max-w-3xl mx-auto text-center space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-center pt-4">
              <a 
                href="https://wa.me/5521979549241?text=Ol%C3%A1,%20gostaria%20de%20informa%C3%A7%C3%B5es%20sobre%20a%20assessoria%20jur%C3%ADdica%20para%20o%20leil%C3%A3o%20da%20casa%20na%20Rua%20J%C3%BAlio%20Otoni%20254%20em%20Santa%20Teresa."
                target="_blank"
                rel="noopener noreferrer"
          ...
          <div className="bg-midnight-light border border-bronze/30 rounded-3xl p-6 sm:p-8 max-w-2xl w-full text-left space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 text-xs sm:text-sm text-text-muted leading-relaxed">
              ...
              <div className="p-4 bg-midnight rounded-xl border border-white/5 space-y-1">
                ...
                <div>2ª Praça (50%): {item.p2Date} — {item.p2Val}</div>
              </div>

              <div className="p-4 bg-midnight rounded-xl border border-white/5 space-y-1">
                <div className="text-white font-semibold">Registros Cartorários:</div>
                <div>Matrícula nº 52318 do 9º RGI/RJ • IPTU nº 0.417.443-9</div>
              </div>
            ...
            <div className="pt-4 border-t border-white/10 flex justify-end gap-3">
              ...
              <a 
                href="https://wa.me/5521979549241?text=Ol%C3%A1,%20gostaria%20de%20solicitar%20o%20parecer%20completo%20do%20edital%20da%20casa%20na%20Rua%20J%C3%BAlio%20Otoni%20254%20em%20Santa%20Teresa."
                target="_blank"
                rel="noopener noreferrer"
...
export default SantaTeresaJulioOtoni254AuctionPage;
