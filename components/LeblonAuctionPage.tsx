...
import SEO from './SEO';
import buildingImage from '../src/assets/images/regenerated_image_1785337087260.png';
import { useLanguage } from '../context/LanguageContext';
import { editalCommon, editaisData } from '../translations/editais';

interface LeblonAuctionPageProps {
  onBack?: () => void;
}

const LeblonAuctionPage: React.FC<LeblonAuctionPageProps> = () => {
  ...
  const editalEntry = editaisData['leblon-general-urquiza'] || editaisData['copacabana-viveiros'];
  const item = editalEntry[language] || editalEntry.pt;

  const canonicalUrl = "https://soaresmartinsadv.com/assessoria-leiloes-judiciais-imoveis-rio-de-janeiro/leblon/apartamento/rua-general-urquiza-263/";
                ...
                <div className="mt-6 pt-4 border-t border-white/10 space-y-3">
                  ...
                  <a 
                    href={`https://wa.me/5521979549241?text=${encodeURIComponent(
                      language === 'es' ? 'Hola, quisiera información sobre la subasta de Leblon en Calle General Urquiza 263.' :
                      language === 'en' ? 'Hello, I would like information about the Leblon auction on General Urquiza St 263.' :
                      'Olá, gostaria de informações sobre o leilão do Leblon na Rua General Urquiza 263.'
                    )}`}
                    target="_blank"
        ...
        <section className="bg-bronze p-8 sm:p-12 md:p-14 rounded-3xl text-midnight text-center shadow-2xl border border-bronze/30">
          <div className="max-w-3xl mx-auto text-center space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-center pt-4">
              <a 
                href={`https://wa.me/5521979549241?text=${encodeURIComponent(
                  language === 'es' ? 'Hola, quisiera asesoría jurídica para la subasta de Leblon en Calle General Urquiza.' :
                  language === 'en' ? 'Hello, I would like legal advice for the Leblon auction on General Urquiza St.' :
                  'Olá, gostaria de informações sobre a assessoria jurídica para o leilão do Leblon na Rua General Urquiza.'
                )}`}
                target="_blank"
                rel="noopener noreferrer"
        ...
        <div className="fixed inset-0 z-[1000] bg-midnight/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-midnight-light border border-bronze/30 rounded-3xl p-6 sm:p-8 max-w-2xl w-full text-left space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="pt-4 border-t border-white/10 flex justify-end gap-3">
              ...
              <a 
                href={`https://wa.me/5521979549241?text=${encodeURIComponent(
                  language === 'es' ? 'Hola, quisiera solicitar el parecer completo sobre el edicto de Leblon.' :
                  language === 'en' ? 'Hello, I would like to request the full legal audit report for the Leblon auction.' :
                  'Olá, gostaria de solicitar o parecer completo do edital do Leblon.'
                )}`}
                target="_blank"
                rel="noopener noreferrer"
...
export default LeblonAuctionPage;
